...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_booking where request_id='" .$_GET["rid"]. "'and booking_status='0'";
                $rs = $Con->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["booking_id"];
                $amt = $_POST["carttotalamt"];
    $_SESSION["carttotalamt"] = $amt;
                $upQry1 = "update tbl_booking set booking_date=curdate(),booking_amount='".$amt."',booking_status='1' where request_id='" .$_GET["rid"]. "'";
				$Con->query($upQry1);
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where b.request_id='" .$_GET["rid"]. "' and booking_status='0' ";
            $res = $Con->query($sel);
                while ($row=$res->fetch_assoc()) {
                    ...
                    if ($rowpr=$respr->fetch_assoc()) {
						...
						$selstock1="select sum(cart_qty) as quantity from tbl_cart where product_id='".$rowpr["product_id"]."' >'0'";
